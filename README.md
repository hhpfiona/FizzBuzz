...
the code are doing. For example, what's the Java version of Python's `and`? It's `&&`. What's going
on with that weird `for` loop, `for (int i = 1; i < 100; i++)`? It could mean many things:

(1) `i = 1, while i < 100, i += 1`
